...
def maxrec():
    ...
    mycur = mydb.cursor()
    mycur.execute("select max(sl_no) from Deposit")
    mydata = mycur.fetchone()
    if mydata[0] is None:
        t1.delete(0, END)
        t1.insert(0, "1")
...
def depos():
    ...
    mydb.commit()
    messagebox.showinfo("Confirm...", "Amount Deposited")
    maxrec()
...
frame = Frame(win, bg="skyblue3", width=515, height=450)
frame.place(x=50, y=80)

l1 = Label(win, text="Deposit Page", font=f0)
l1.place(x=150, y=50)
...
l7 = Label(win, text="Balance :", font=("Arial", 18, 'bold'), bg="skyblue3")
...
b3 = Button(win, text="DEPOSIT", bd=5, font=f1, command=depos)
b3.place(x=250, y=450)

b5 = Button(win, text="EXIT", bd=5, font=f1, command=exit_fun)
b5.place(x=370, y=450)
